...
export const fetchPokemonList = async (limit = 1025, offset = 0) => {
  const url = `${BASE_URL}/?limit=${limit}&offset=${offset}`;
  try {
    const response = await axios.get<PokemonListResponse>(url);
    return response.data.results;
  } catch (error: any) {
    throw new Error(error.message || "Failed to fetch Pokémon list.");
  }
};

const pokemonCache: Record<string, PokemonDetails> = {};

export const fetchPokemonDetails = async (idOrName: number | string) => {
  ...
  try {
    ...
    pokemonCache[idOrName] = response.data;
    return response.data;
  } catch (error: any) {
    throw new Error(
      error.message || `Failed to fetch Pokémon details for ${idOrName}.`
    );
  }
};
